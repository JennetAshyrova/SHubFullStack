namespace Konu17HataYonetimi
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            {     //  exception bir isim vererek hatayı yakalayabiliriz
                Console.WriteLine("Hata oluştu! lütfen sadece sayısal değer giriniz ");

                throw;  // throw yinw hata fırlatılmasını sağlar 
             Console.WriteLine(hata.Message);  // hata nesnesi içinde hata detayları yer alır
                // ayrıca uygulamalarda hata oluşursa oluşan hatalar veritabanı ve ya viziksel bir dosyaya kaydedilir , bu işleme loglama denir
             }
            finally // isteğe bağlı son blok
            {
                Console.WriteLine("try catch bloğundan sonra her seferinde çalışmasını istediğimiz bir işlem varsa bu blokta çalıştırabilirz.Kullanımı zorunlu değildir");
                Console.WriteLine("Kdv hesaplamak için fiyat girini:");
